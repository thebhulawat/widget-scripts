...
(() => {
  ...
  try {
    let profileIconMmid: string | null = null;

    async function fetchProfileMmid(): Promise<string> {
    ...
    function clickProfileIcon(): void {
      ...
      const profileIcon = document.querySelector<HTMLButtonElement>(`button[mmid="${profileIconMmid}"]`);
      if (profileIcon) {
        profileIcon.click();
    ...
    function injectMmidAttributes(): void {
